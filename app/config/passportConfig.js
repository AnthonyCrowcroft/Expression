...
var configAuth = config.passportAuthConfig;
    ...
    function(req, email, password, done) {
        ...
        process.nextTick(function() {
            User.findOne({ 'local.email' :  email }, function(err, user) {
                ...
                if (!user)
                    return done(null, false, {type:"warn", msg: "No user found"});

                if (!user.validPassword(password))
                    return done(null, false, {type:"danger", msg: "Oops! Wrong password"});

                // all is well, return user
                else
                    if (user == req.user)
                        return done(null, false,{type:"info", msg: "Logged in"});
                    else
                        return done(null, user, {type:"info", msg: "Logged in"});
            });
        });
